...
class wish extends conexion {
	...
	public  function login($usuario, $pass){
		$this->user = $usuario;
		$this->password= $pass;
		$this->consulta= mysql_query("SELECT usuario , password FROM personal where usuario = '".$this->user."' and  password = '".$this->password."'", $this->conexion); 
                if($this->row = mysql_fetch_array($this->consulta)){
                    session_start();
                    $this->consulta = mysql_query("SELECT * FROM personal where usuario = '".$this->user."' ",  $this->conexion);  
                    $this->row = mysql_fetch_array($this->consulta);
                    //Id
                    $this->id = $this->row['personal_id'];
                    $_SESSION['id'] = $this->id;
                    $this->ini = 1;
                    //Nombre
